...
setTimeout(()=>{
  ...
  ok(/^data:image\/png;base64,/.test(buddha.getAttribute("src")),
     "embedded inline, not fetched from anywhere");
  ok(buddha.getAttribute("src").length>1000,"a real photo, not a stub");
  ok([...w.document.querySelectorAll(".buddha-mark")].length===1,
     "he appears once, in the same panel, and nowhere else");

  console.log("\n-- the tracker still works --");
  ok(w.document.querySelectorAll("#rows > .row, #petrows > .row").length===15,"15 markers still render");
